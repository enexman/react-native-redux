...
class HitPointLineMonster extends React.Component {
  render() {
    const { monster, theme } = this.props;
    const totalWidth = 240;
    const fullLine = totalWidth/100 * (monster.hitPoint/monster.fullPoint * 100);
    const woundLine = totalWidth/100 * (monster.wound/monster.fullPoint * 100);
    ...
    return (
      <View style={styles.container}>
        <Text style={[styles.text, theme.color]}>{monster.name.toUpperCase()}: </Text>
...
export default connect(({monster, theme}) => ({monster, theme}), {})(HitPointLineMonster);
